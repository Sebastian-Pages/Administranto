    ...
    return (
        <div className={`${matched ? '' : 'opacity-10'}`}>
	        ...
	        <Draggable draggableId={id} index={index}>
	            {(provided, snapshot) => 
	                <div onClick={()=>setModal(true)} {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef} className={`shadow-lg transition-shadow duration-300 hover:shadow-xl mb-4 rounded px-1.5 py-2.5 ${snapshot.isDragging ? 'bg-gradient-to-r from-red-50 to-blue-50 text-gray-900' : 'bg-white text-gray-800'}`}>
